...
export default function OrdersPage() {
    ...
    return (
      <div className="text-center py-16">
        <div className="mx-auto h-14 w-14 rounded-full bg-stone-100 flex items-center justify-center">
          <Icon name="package" className="h-7 w-7 text-stone-400" />
        </div>
  ...
  return (
    <div>
      <h1 className="text-xl font-medium text-stone-900 mb-4">Your orders</h1>
      <div className="space-y-2">
        {orders.map((order) => (
          ...
          >
            <div>
              <div className="font-medium text-stone-900">Order #{order.id.slice(0, 8)}</div>
            ...
            <div className="text-right">
              ...
              <div className="text-sm font-medium text-accent mt-1">₹{order.totalAmount}</div>
            </div>
          </Link>
        ))}
      </div>
