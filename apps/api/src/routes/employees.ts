...
import { z } from "zod";
import { businessDate, validDate } from "@matrizo/shared";
  ...
  const [monthTotals] = await db
    ...
    .where(
      and(
        ...
        gte(salesEntries.date, `${monthPrefix}-01`),
        lte(salesEntries.date, `${monthPrefix}-31`),
      ),
    );
...
employeeRoutes.get("/me", async (c) => {
  ...
  const summary = await commissionSummary(
    db,
    auth.sub,
    row.employee_profiles.commissionRatePercent,
  );
  return c.json({
    employee: {
      id: row.users.id,
...
employeeRoutes.patch("/me", async (c) => {
  ...
  if (Object.keys(profileFields).length > 0) {
    await db
      ...
      .set(profileFields)
      .where(eq(employeeProfiles.userId, auth.sub));
  }
...
employeeRoutes.get("/me/sales", async (c) => {
  ...
  if (month && !validDate(`${month}-01`))
    return c.json({ error: "Enter a valid month." }, 400);
  if (month) {
    conditions.push(
      gte(salesEntries.date, `${month}-01`),
      lte(salesEntries.date, `${month}-31`),
    );
  }
...
const upsertSalesSchema = z.object({
  amount: z.number().finite().nonnegative().max(100000000).multipleOf(0.01),
...
employeeRoutes.put("/me/sales/:date", async (c) => {
  ...
  if (!validDate(date) || date > businessDate())
    return c.json(
      { error: "Enter a valid sales date, today or earlier." },
      400,
    );
  const parsed = upsertSalesSchema.safeParse(
    await c.req.json().catch(() => null),
  );
  if (!parsed.success)
    return c.json(
  ...
  await db
    ...
    .onConflictDoUpdate({
      target: [salesEntries.userId, salesEntries.date],
      set: {
        amount: parsed.data.amount,
        notes: parsed.data.notes ?? null,
        updatedAt: new Date(),
...
// A daily team report includes inactive employees so historical totals stay complete.
employeeAdminRoutes.get("/sales", async (c) => {
  const date = c.req.query("date") ?? businessDate();
  if (!validDate(date)) return c.json({ error: "Enter a valid date." }, 400);
  const db = getDb(c.env.DB);
  const entries = await db
    .select({
      ...
      active: users.active,
      commissionRatePercent: employeeProfiles.commissionRatePercent,
    })
    .from(salesEntries)
    .innerJoin(users, eq(salesEntries.userId, users.id))
    .innerJoin(employeeProfiles, eq(users.id, employeeProfiles.userId))
    .where(and(eq(salesEntries.date, date), eq(users.role, "sales_employee")))
    .orderBy(desc(salesEntries.amount));
  const activeEmployees = await db
    .select({ id: users.id, name: users.name })
    .from(users)
    .where(and(eq(users.role, "sales_employee"), eq(users.active, true)));
  const submitted = new Set(entries.map((e) => e.employeeId));
  return c.json({
    date,
    entries,
    summary: {
      totalSales:
        Math.round(entries.reduce((sum, e) => sum + e.amount, 0) * 100) / 100,
      totalCommission: entries.reduce(
        (sum, e) => sum + commissionFor(e.amount, e.commissionRatePercent),
        0,
      ),
      submitted: activeEmployees.filter((e) => submitted.has(e.id)).length,
      activeEmployees: activeEmployees.length,
      missing: activeEmployees.filter((e) => !submitted.has(e.id)),
    },
  });
});
...
employeeAdminRoutes.get("/", async (c) => {
  ...
  const employees = await Promise.all(
    rows.map(async (row) => ({
      ...
      phone: row.employee_profiles.phone,
      commission: await commissionSummary(
        db,
        row.users.id,
        row.employee_profiles.commissionRatePercent,
      ),
...
employeeAdminRoutes.post("/", async (c) => {
  ...
  await db.batch([
    db
      .insert(users)
      .values({ id, role: "sales_employee", email, passwordHash, name }),
    db.insert(employeeProfiles).values({
      userId: id,
      phone: phone ?? null,
...
employeeAdminRoutes.get("/:id", async (c) => {
  ...
  const db = getDb(c.env.DB);
  const row = await loadProfile(db, id);
  if (!row || row.users.role !== "sales_employee")
    return c.json({ error: "Employee not found" }, 404);

  const entries = await db
    .select()
    .from(salesEntries)
    .where(eq(salesEntries.userId, id))
    .orderBy(desc(salesEntries.date));
  const commission = await commissionSummary(
    db,
    id,
    row.employee_profiles.commissionRatePercent,
  );
...
employeeAdminRoutes.delete("/:id", async (c) => {
  const db = getDb(c.env.DB);
  const id = c.req.param("id")!;
  const [employee] = await db
    .select()
    .from(users)
    .where(and(eq(users.id, id), eq(users.role, "sales_employee")))
    .limit(1);
  if (!employee) return c.json({ error: "Employee not found" }, 404);
  await db.update(users).set({ active: false }).where(eq(users.id, id));
  return c.json({ ok: true, retainedSalesHistory: true });
});
...
employeeAdminRoutes.patch("/:id", async (c) => {
  ...
  if (contactAddress !== undefined)
    profileUpdate.contactAddress = contactAddress;
  if (commissionRatePercent !== undefined)
    profileUpdate.commissionRatePercent = commissionRatePercent;
