...
export default function ProfessionalsPage() {
  ...
  const [profiles, setProfiles] = useState<ManagedProfessional[]>([]);
  ...
  async function load() {
    ...
    try {
      setProfiles(
        (
          await api.get<{ professionals: ManagedProfessional[] }>(
            "/team/professionals",
          )
        ).professionals.filter((p) => p.kind === kind),
      );
    } catch {
  ...
  useEffect(() => {
    ...
    let live = true;
    api
      .get<{ professionals: ManagedProfessional[] }>("/team/professionals")
      .then((r) => {
        if (live) {
          setProfiles(r.professionals.filter((p) => p.kind === kind));
          setError("");
        }
      })
      .catch(() => {
  ...
  if (!allowed) return <p>Administrator sign-in required.</p>;
  return (
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {profiles.map((profile) => (
          <article key={profile.id} className="border rounded p-4 space-y-3">
            ...
            <ProfessionalAccount profile={profile} onSaved={load} />
          </article>
        ))}
      </div>
    </div>
