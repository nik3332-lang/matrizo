...
const FORWARD_STATUSES: OrderStatus[] = [
  "placed",
  "confirmed",
  "picked",
  "dispatched",
...
export default function AdminOrderDetailPage({
  ...
  useEffect(() => {
    if (user?.role === "admin" || user?.role === "store_staff") {
      // Non-blocking: only feeds the delivery-partner assignment dropdown,
      // so a failure here shouldn't stop the rest of the order page from
      // working — just leaves that dropdown empty instead of unhandled.
      api
        .get<{ users: DeliveryPartner[] }>("/admin/users")
    ...
    try {
      ...
      setError(
        err instanceof ApiError
          ? err.message
          : "Could not assign delivery partner.",
      );
    } finally {
      setUpdating(false);
    }
  ...
  const nextStatus =
    currentIndex >= 0 && currentIndex < FORWARD_STATUSES.length - 1
      ? FORWARD_STATUSES[currentIndex + 1]
      : null;
  const canCancel =
    canManageOrders && ["placed", "confirmed"].includes(order.status);
  const canMarkDelivered = isDeliveryPartner && order.status === "dispatched";

  const eligiblePartners = (partners ?? []).filter(
    (p) => p.role === "delivery_partner" && p.storeId === order.storeId,
  );
  ...
  return (
    <div className="max-w-lg">
      ...
      <div className="mt-4 flex items-center gap-3 flex-wrap">
        <span
          className={`rounded-full text-xs px-3 py-1.5 font-medium capitalize ring-1 ${STATUS_COLORS[order.status]}`}
        >
          {order.status}
        </span>
      ...
      {address && (
        <section className="glass mt-6 rounded-xl p-4 text-sm">
          ...
          {customer?.phone && (
            <a
              className="inline-block mt-2 text-accent"
            ...
            >
              <option value="" disabled>
                {eligiblePartners.length === 0
                  ? "No delivery partners at this store"
                  : delivery
                    ? "Reassign to…"
                    : "Assign to…"}
              </option>
